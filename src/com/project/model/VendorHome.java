...
/**
 * Home object for domain model class Vendor.
 * @see com.project.model.Vendor
 * @author Hibernate Tools
 */
public class VendorHome {

	private static final Log log = LogFactory.getLog(VendorHome.class);
	...
	public void persist(Vendor transientInstance) {
		log.debug("persisting Vendor instance");
		try {
	...
	public Vendor merge(Vendor detachedInstance) {
		...
		try {
			Vendor result = (Vendor) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Vendor findById(java.math.BigDecimal id) {
		log.debug("getting Vendor instance with id: " + id);
		try {
			Vendor instance = (Vendor) sessionFactory.getCurrentSession().get("com.project.model.Vendor", id);
			if (instance == null) {
	...
	public List findByExample(Vendor instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("com.project.model.Vendor")
